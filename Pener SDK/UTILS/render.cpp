...
namespace RENDER
{
	...
	void DrawPixel(int x, int y, CColor col)
	{
	...
	void DrawGradientRectangle(Vector2D Position, Vector2D Size, CColor Top, CColor Bottom) {
		INTERFACES::Surface->DrawSetColor(Top);
		INTERFACES::Surface->DrawFilledRectFade(Position.x, Position.y, Position.x + Size.x, Position.y + Size.y, 255, 255, false);

		INTERFACES::Surface->DrawSetColor(Bottom);
		INTERFACES::Surface->DrawFilledRectFade(Position.x, Position.y, Position.x + Size.x, Position.y + Size.y, 0, 255, false);
	}
